    ...
    private async resolveQuickPickDialog(cdpService: CdpService): Promise<boolean> {
        ...
        while (Date.now() - started < maxWaitMs) {
            ...
            for (const ctx of contexts) {
                try {
                    ...
                    if (val && val.found && typeof val.x === 'number' && typeof val.y === 'number') {
                        logger.info(`[ChatSessionService] Resolving QuickPick (${val.type}) at x=${val.x}, y=${val.y} in context ${ctx.id}: ${val.text}`);
            ...
            if (resolvedAny) {
                ...
                if (!remains) {
                    logger.info(`[ChatSessionService] All QuickPick dialogs resolved successfully.`);
                    return true;
                }
            }
