...
**КРИТИЧЕСКИЙ ЗАПРЕТ:** Запрещено открывать и читать файлы ниже проактивно "на всякий случай". Открывай их только по явному запросу пользователя, при самоотладке или при необходимости решить конкретную задачу, описанную в назначении файла.

*   [CLAUDE.md](file:///e:/Desktop/Remoat/CLAUDE.md) — Общая справка по проекту для AI-ассистентов (команды запуска, архитектура, пути к файлам). Читать при старте работы с проектом для быстрого вхождения.
*   [CONTRIBUTING.md](file:///e:/Desktop/Remoat/CONTRIBUTING.md) — Руководство по контрибьютингу, стилю кода, структуре директорий и написанию коммитов. Открывать перед коммитом или при создании нового кода.
*   [SECURITY.md](file:///e:/Desktop/Remoat/SECURITY.md) — Политика и модель безопасности Remoat (локальный запуск, CDP, whitelist). Открывать при изменении логики аутентификации, сети или прав доступа.
*   [CODE_OF_CONDUCT.md](file:///e:/Desktop/Remoat/CODE_OF_CONDUCT.md) — Кодекс поведения участников. Открывать при вопросах взаимодействия в сообществе.
*   [docs/ARCHITECTURE.md](file:///e:/Desktop/Remoat/docs/ARCHITECTURE.md) — Подробное описание трехзвенной архитектуры проекта (CLI -> Bot -> Services/DB). Читать перед рефакторингом крупных модулей.
*   [docs/ANTIGRAVITY_DOM_SELECTORS.md](file:///e:/Desktop/Remoat/docs/ANTIGRAVITY_DOM_SELECTORS.md) — Справочник селекторов DOM для взаимодействия с Antigravity. Читать при отладке или модификации CDP-кликов, кнопок Undo/Stop и текстовых полей.
*   [docs/RESPONSE_MONITOR.md](file:///e:/Desktop/Remoat/docs/RESPONSE_MONITOR.md) — Логика работы фонового мониторинга ответа (дублирующие потоки, детекторы, тайминги). Читать при изменении `responseMonitor.ts` или фильтрации вывода.
*   [docs/dom-inspection-guide.md](file:///e:/Desktop/Remoat/docs/dom-inspection-guide.md) — Руководство по ручной и автоматической инспекции DOM. Читать при необходимости обновить или найти новые селекторы IDE.
```