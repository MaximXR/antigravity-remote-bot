...
- **ОБЯЗАТЕЛЬНО** использовать нечеткое сопоставление имен проектов (без учета эмодзи, лишних пробелов и регистра) при обработке событий переподключения, восстановления или выбора окон IDE, чтобы избежать багов с несовпадением префиксов (например, эмодзи `🗂️` в мультипапках).
- **ОБЯЗАТЕЛЬНО** следить за размером `callback_data` для инлайн-кнопок Telegram (строгий лимит 64 байта). Запрещено передавать полные пути файлов или длинные имена проектов в `callback_data` напрямую; вместо этого использовать короткие ID и кэшировать данные во внутреннем состоянии бота.
